...
do
	...
	MovementRightSection:AddSlider({
		...
		Callback = function(value)
			...
			showNotification("Movement", "Speed: " .. value)
		end,
	})
	MovementRightSection:AddToggle({
		Name = "Fly & Noclip",
...
do
	...
	local WorldAmbientSection =
		WorldVisualsTab:DrawSection({ Name = "World Ambient", Position = "left", EnableScrolling = true })
	local CharacterColorsSection =
		WorldVisualsTab:DrawSection({ Name = "Character Colors", Position = "right", EnableScrolling = true })
	local AuraSection =
		WorldVisualsTab:DrawSection({ Name = "Transform Aura", Position = "left", EnableScrolling = true })
	...
	do
		...
		AuraSection:AddButton({
			...
			Callback = function()
				ResetAurasToDefault()
				showNotification("Aura System", "Aura colors reset to default!")
			end,
		})
	end
	do
...
do
	...
	do
		...
		UserInputService.InputBegan:Connect(function(input, gp)
			...
			if quickOrbitKeybind and quickOrbitKeybind ~= "NIL" and input.KeyCode.Name == quickOrbitKeybind then
				...
				if isQuickOrbiting then
					...
					orbitEnabled = false
					stopOrbit()
					showNotification("Utility", "Orbit OFF")
				end
			end
			if
		...
		UtilityLeftSection:AddDropdown({
			...
			Callback = function(v)
				...
				showNotification("Utility", "Orbit target: " .. name)
			end,
		})
	...
	do
		...
		UtilityRightSection:AddToggle({
			Name = "Enable Char Swap",
			Flag = "CharSwapEnabled",
			Default = Cfg.CharSwapEnabled,
			Callback = function(v)
				Cfg.CharSwapEnabled = v
				setupKeybinds()
				showNotification("Utility", "Quick Swap: " .. (v and "ON" or "OFF"))
			end,
		})
		local keyOptions = {
			"NIL",
		...
		UtilityRightSection:AddButton({
			...
			Callback = function()
				...
				if #valid > 0 then
					...
					showNotification("Utility", "No servers found")
				end
			end,
		})
	end
...
do
	local UISettingsTab =
		Window:DrawTab({ Name = "UI Settings", Icon = "settings-3", Type = "Single", EnableScrolling = true })
